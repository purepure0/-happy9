...
class CommunityOlderCareViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func getData(type:String){
        AppRequestManager.shareManager.getOrganizationList("", latitude: "", longitude: "", type: type) { (success, response) in
            if success {
    ...
    func tableView(tableView: UITableView, numberOfRowsInSection section: Int) -> Int{
        if self.userInfor != nil {
            return (self.userInfor?.array?.count)!
        }else{
            return 0
        }
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell{
        ...
        cell.setValueWithInfo((self.userInfor?.array![indexPath.row])!)
